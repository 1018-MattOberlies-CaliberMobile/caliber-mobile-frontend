...
describe('auto overall status selector', () => {
  it('picks an overall technical status based on individual scores', () => {
    const sample1 = [1, 1, 2, 3, 4, 2, 0, 3, 1, 1, 2, 3, 0] as TechnicalScore[];
    const sample2 = [0, 0, 0, 0, 1, 2, 3] as TechnicalScore[];
    const sample3 = [3, 4, 4, 4, 3, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4] as TechnicalScore[];
    const sample4 = [1, 2, 1, 4, 1, 1, 1, 1, 1, 1, 2, 0, 0, 0, 1, 1, 1] as TechnicalScore[];
    const sample5 = [3, 1, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 4, 1, 0] as TechnicalScore[];

    expect(autoSelector(sample1)).toBe(2 as TechnicalScore);
    expect(autoSelector(sample2)).toBe(0 as TechnicalScore);
    expect(autoSelector(sample3)).toBe(4 as TechnicalScore);
    expect(autoSelector(sample4)).toBe(1 as TechnicalScore);
